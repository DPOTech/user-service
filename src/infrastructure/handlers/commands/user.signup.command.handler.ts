...
export class UserSignUpCommandHandler implements ICommandHandler<UserSignUpCommand>{
    constructor(@Inject('IUserService') private service: IUserService, @Inject('IUserEventStoreService') private eventStoreService: IUserEventStoreService, private publisher: EventPublisher) {

    }
    async execute(command: UserSignUpCommand): Promise<Message> {
        let existedUser = await this.eventStoreService.readStreamAsync(command.userName);
        if (existedUser)
